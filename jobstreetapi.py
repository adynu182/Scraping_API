...
def save_to_xlsx(df):
    df.to_excel(f'{jlmrow}_jobs.xlsx', index=False)
...
def job_detail(id):  
    ...
    if resp['data']['jobDetail'] is not None:
        ...
        link = ""
        job_none.append(
        (nama, jobtitle, status, pendidikan, bahasa, alamat, loks, gaji, level, pengalaman, size, deskripsi, postdate, closedate, industri, link)
        )        
        return job_none

    job_detail = []
    nm = row['header']['company']
    if 'name' in nm or 'name' == None or 'name' == "":
        name = "kosong"
    else:
        name = row['header']['company']['name']
    ...
    bahasa = row['jobDetail']['jobRequirement']['languages']
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(all_id, columns=['Nama Perusahaan', 'Posisi', 'Status Pegawai', 'Pendidikan', 'Bahasa', 'Alamat Kantor', 'Lokasi/Penempatan', 'Gaji Range', 'Level', 'Pengalaman', 'Jumlah Karyawan', 'Deskripsi', 'Tanggal Posting', 'Dated', 'Jenis Perusahaan', 'Link'])
    print(df)
